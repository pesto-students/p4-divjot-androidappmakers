  ...
  try {
    ...
    for (const element of cities) {
      let cityWeatherResponse = await fetch(
        `${BASE_URL}${WEATHER_API.CURRENT}?key=6208cdb88a214eca92a62115221411&q=${element}&aqi=no`
      );
...
export const getForecast = async ({res, city, days}) => {
  try {
    const forecastResponse = await fetch(
      `${BASE_URL}${WEATHER_API.FORECAST}?key=6208cdb88a214eca92a62115221411&q=${city}&days=${days}&aqi=no&alerts=no`
    );
...
export const getCurrentWeather = async ({res, city}) => {
  try {
    let cityWeatherResponse = await fetch(
      `${BASE_URL}${WEATHER_API.CURRENT}?key=6208cdb88a214eca92a62115221411&q=${city}&aqi=no`
    );
    let cityWeather = await cityWeatherResponse.json();
    if (cityWeather.error) {
      res.writeHead(400, {"Content-Type": "application/json"});
